...
// Brand ID for Diety od Brokuła
const BRAND_ID = "9f9d0061-6916-4f30-8057-c6da5dac85c3";
const BRAND_NAME = "Diety od Brokuła";

const companyInfo = {
  name: "Diety od Brokuła",
  brandName: "Diety od Brokuła",
  businessType: "Catering dietetyczny",
  voivodeship: "Ogólnopolski",
  website: "www.dietyodbrokula.pl",
  description: "Jeden z wiodących dostawców cateringu dietetycznego w Polsce, oferujący szeroki wybór diet dopasowanych do różnych potrzeb żywieniowych.",
};
...
export default function DietyOdBrokulaSummary2025() {
  ...
  // Fetch review aspects for use in executive summary
  const { data: aspectsData } = useReviewAspects(BRAND_ID);
  ...
  const { data: periodStats, isLoading: isLoadingStats } = useQuery({
    queryKey: ['dietyodbrokula-period-stats', 'v3-last-full-month'],
    queryFn: async () => {
  ...
  const { data: monthlyTrends, isLoading: isLoadingTrends } = useQuery({
    queryKey: ['dietyodbrokula-monthly-trends', 'v3-full-range'],
    queryFn: async () => {
      const allReviews = await fetchAllReviews(BRAND_ID);
  ...
  const { data: dateRange } = useQuery({
    queryKey: ['dietyodbrokula-date-range'],
    queryFn: async () => {
  ...
  const { data: crisisMonthsData } = useQuery({
    queryKey: ['dietyodbrokula-crisis-months'],
    queryFn: async () => {
  ...
  const { data: realQuotes, isLoading: isLoadingQuotes } = useQuery({
    queryKey: ['dietyodbrokula-real-quotes'],
    queryFn: async () => {
  ...
  const { data: discountComparison, isLoading: isLoadingDiscounts } = useQuery({
    queryKey: ['dietyodbrokula-discount-comparison-q4-v2'],
    queryFn: async () => {
  ...
  const { data: activeCodes, isLoading: isLoadingCodes } = useQuery({
    queryKey: ['dietyodbrokula-discount-codes-q4'],
    queryFn: async () => {
      const { data } = await supabase
        .from('discounts')
  ...
  const { data: priceComparison, isLoading: isLoadingPrices } = useQuery({
    queryKey: ['dietyodbrokula-price-comparison-q4', 'v5-daily-stats'],
    queryFn: async () => {
      ...
      // Keep chart readable: show top 12, but always include Diety od Brokuła even if outside top
      const brokulEntry = allCatalogPrices.find(p => p.brandId === BRAND_ID);
      let catalogPrices = allCatalogPrices.slice(0, 12);
      if (brokulEntry && !catalogPrices.some(p => p.brandId === BRAND_ID)) {
        catalogPrices = [...catalogPrices.slice(0, 11), brokulEntry];
      }
  ...
  // Generate dynamic key insight based on actual aspect data
  const keyInsight = useMemo(() => {
    if (!aspectsData || aspectsData.length === 0) return 'analiza aspektów w toku...';
    
    const qualityAspect = aspectsData.find(a => a.aspect === 'jakość');
    const tasteAspect = aspectsData.find(a => a.aspect === 'smak');
    const deliveryAspect = aspectsData.find(a => a.aspect === 'dostawa');
    const priceAspect = aspectsData.find(a => a.aspect === 'cena');
    
    const issues: string[] = [];
    const strengths: string[] = [];
    
    // Check each aspect for issues (positive < 70%) or strengths (positive >= 80%)
    if (qualityAspect) {
      if (qualityAspect.positive < 70) issues.push(`jakość (${qualityAspect.positive}% pozytywnych)`);
      else if (qualityAspect.positive >= 80) strengths.push('jakość');
    }
    if (tasteAspect) {
      if (tasteAspect.positive < 70) issues.push(`smak (${tasteAspect.positive}% pozytywnych)`);
      else if (tasteAspect.positive >= 80) strengths.push('smak');
    }
    if (deliveryAspect) {
      if (deliveryAspect.positive < 70) issues.push(`dostawa (${deliveryAspect.positive}% pozytywnych)`);
      else if (deliveryAspect.positive >= 80) strengths.push('dostawa');
    }
    if (priceAspect) {
      if (priceAspect.positive < 70) issues.push(`cena (${priceAspect.positive}% pozytywnych)`);
      else if (priceAspect.positive >= 80) strengths.push('konkurencyjne ceny');
    }
    
    if (issues.length > 0) {
      return `obszary wymagające poprawy: ${issues.join(', ')}`;
    } else if (strengths.length > 0) {
      return `mocne strony: ${strengths.join(', ')}`;
    } else {
      return 'stabilna pozycja na rynku z potencjałem do poprawy';
    }
  }, [aspectsData]);

  // Find brand's position in discount comparison
  const brandDiscount = useMemo(() => {
    if (!discountComparison) return null;
    return discountComparison.find((d: any) => d.brandId === BRAND_ID);
  }, [discountComparison]);

  // Find brand's position in price comparison
  const brandPrice = useMemo(() => {
  ...
  return (
    <div className="min-h-screen bg-background">
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-start gap-6">
              <div className="h-20 w-20 rounded-lg bg-white p-2 flex items-center justify-center shadow-sm overflow-hidden">
                <img 
                  src="https://jpwabrhowkjmuaxnnfhk.supabase.co/storage/v1/object/public/brand-logos/logos/1759140593877-r0snpen.jpeg" 
      ...
      <div className="container mx-auto px-4 py-8 space-y-10">
          ...
          <Card className="border-l-4 border-l-primary bg-primary/5">
            <CardContent className="pt-6">
              <p className="text-lg leading-relaxed">
                ...
                <span className="text-primary font-semibold"> Kluczowe: {keyInsight}.</span>
              </p>
            </CardContent>
          </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex gap-3">
                  ...
                  <p className="text-sm"><strong>Pozycja rynkowa</strong> - {brandPrice ? `${brandPrice.avgPrice} zł średnio` : 'konkurencyjna oferta'}</p>
                </div>
                <div className="flex gap-3">
                ...
                <div className="flex gap-3">
                  ...
                  <p className="text-sm"><strong>Średni rabat:</strong> {brandDiscount?.avgDiscount || 0}% w Q4 2025</p>
                </div>
              </CardContent>
            </Card>
          </div>
        ...
        {/* Sekcja 2: Aspekty opinii */}
        <ReviewAspectsSection />

        <Separator />

        {/* Sekcja 3: Rating Timeline */}
        <section className="space-y-6">
          <h2 className="text-2xl font-bold flex items-center gap-2">
          ...
          <Card>
            ...
            <CardContent>
              {isLoadingDiscounts ? (
                ...
                (() => {
                  // Always include brand in the chart, even if outside top 10
                  const brandEntry = discountComparison.find((d: any) => d.brandId === BRAND_ID);
                  let chartData = discountComparison.slice(0, 10);
                  if (brandEntry && !chartData.some((d: any) => d.brandId === BRAND_ID)) {
                    chartData = [...chartData.slice(0, 9), brandEntry].sort((a: any, b: any) => b.avgDiscount - a.avgDiscount);
                  }
                  return (
                ...
                <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                  {activeCodes.slice(0, 6).map((code, idx) => (
                    <div key={idx} className="p-4 rounded-lg border bg-card">
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        {code.valid_from ? format(new Date(code.valid_from), 'dd.MM.yyyy') : '?'} - {code.valid_until ? format(new Date(code.valid_until), 'dd.MM.yyyy') : '?'}
                      </p>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-center text-muted-foreground py-4">Brak aktywnych kodów rabatowych w Q4 2025</p>
              )}
            </CardContent>
          </Card>
        </section>
        ...
        <section className="space-y-6">
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Średnia cena w Q4 2025</CardDescription>
              </CardHeader>
              <CardContent>
                {isLoadingPrices ? (
                  <Skeleton className="h-[350px] w-full" />
                  ...
                  <div className="h-[350px]">
                    <ResponsiveContainer width="100%" height="100%">
                      ...
                      >
                        ...
                        <YAxis dataKey="brand" type="category" width={105} tick={{ fontSize: 11 }} />
                        <Tooltip formatter={(value: number) => [`${value} zł`, 'Średnia cena']} />
                  ...
                  <p className="text-center text-muted-foreground py-8">Brak danych cenowych dla Q4 2025</p>
                )}
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Średnia cena po zastosowaniu rabatu</CardDescription>
              </CardHeader>
              <CardContent>
                {isLoadingPrices ? (
                  <Skeleton className="h-[350px] w-full" />
                  ...
                  <div className="h-[350px]">
                    <ResponsiveContainer width="100%" height="100%">
                      ...
                      >
                        ...
                        <XAxis type="number" tickFormatter={(v) => `${v} zł`} />
                        <YAxis dataKey="brand" type="category" width={105} tick={{ fontSize: 11 }} />
                        <Tooltip formatter={(value: number) => [`${value} zł`, 'Cena po rabacie']} />
                        <Bar dataKey="avgPrice" radius={[0, 4, 4, 0]}>
          ...
          <Card className="border-l-4 border-l-primary bg-primary/5">
            <CardContent className="pt-4">
              <p className="text-sm">
                <strong>Insight cenowy:</strong> {brandPrice ? (
                  <>
                    {BRAND_NAME} ze średnią ceną {brandPrice.avgPrice} zł {brandPrice.avgPrice > 60 ? 'pozycjonuje się w segmencie premium' : brandPrice.avgPrice > 45 ? 'plasuje się w środku stawki cenowej' : 'oferuje konkurencyjne ceny'}. 
                    {brandDiscount?.avgDiscount && brandDiscount.avgDiscount > 0 ? ` Ze średnim rabatem ${brandDiscount.avgDiscount}% efektywna cena jest jeszcze bardziej atrakcyjna.` : ''}
                  </>
                ) : (
                  "Brak wystarczających danych cenowych do analizy."
                )}
              </p>
            </CardContent>
          </Card>
        ...
        {/* Sekcja końcowa: Podsumowanie roku */}
        <section className="space-y-6">
          <h2 className="text-2xl font-bold flex items-center gap-2">
            <Sparkles className="h-6 w-6 text-primary" />
          ...
          <Card className="border-2 border-primary/30">
            <CardContent className="pt-6">
              <div className="space-y-4">
                <p className="text-lg">
                  <strong>{BRAND_NAME}</strong> kończy rok 2025 z {parseFloat(allTimeStats?.avgRating || '0') >= 4.5 ? 'doskonałymi wynikami' : parseFloat(allTimeStats?.avgRating || '0') >= 4.0 ? 'dobrymi wynikami' : 'wynikami wymagającymi poprawy'}:
                </p>
                <ul className="space-y-2 ml-4">
                  <li className="flex items-start gap-2">
                    <Star className="h-4 w-4 text-yellow-500 mt-1 flex-shrink-0" />
                    <span>Średnia ocena <strong>{allTimeStats?.avgRating}/5</strong> na podstawie {allTimeStats?.count?.toLocaleString('pl-PL')} opinii</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <ThumbsUp className="h-4 w-4 text-green-500 mt-1 flex-shrink-0" />
                    <span><strong>{allTimeStats?.positivePercent}%</strong> zadowolonych klientów</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <TrendingUp className="h-4 w-4 text-blue-500 mt-1 flex-shrink-0" />
                    <span>Stabilna jakość przez ostatni kwartał</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <Percent className="h-4 w-4 text-primary mt-1 flex-shrink-0" />
                    <span>Średni rabat <strong>{brandDiscount?.avgDiscount || 0}%</strong> w Q4 2025</span>
                  </li>
                </ul>
              </div>
            </CardContent>
