...
// Brand ID for SuperMenu NEW 25
const BRAND_ID = "94180639-4145-41fb-8714-da5a1eee6bc8";

// Competitor brand IDs for comparison
const COMPETITOR_BRANDS: Record<string, string> = {
  "Nice To Fit You": "b5c5a5d2-e7d3-4c0e-8e2a-3f5b7e9c1d4a",
  "MaczFit": "3d542f8b-1e2b-4f3e-8c4b-6d9e2f7a5c8b",
  "Kuchnia Vikinga": "1a2b3c4d-5e6f-7a8b-9c0d-1e2f3a4b5c6d",
  "5 Posiłków": "7f8e9d0c-1b2a-3c4d-5e6f-7a8b9c0d1e2f",
  "BodyChief": "2c3d4e5f-6a7b-8c9d-0e1f-2a3b4c5d6e7f",
  "Pomelo": "4e5f6a7b-8c9d-0e1f-2a3b-4c5d6e7f8a9b",
  "Gastropaczka": "8a9b0c1d-2e3f-4a5b-6c7d-8e9f0a1b2c3d",
};

const companyInfo = {
  name: "SuperMenu NEW 25",
  brandName: "SuperMenu NEW 25",
  businessType: "Catering dietetyczny",
  voivodeship: "Mazowieckie",
  website: "www.supermenu.pl",
  description: "Dynamicznie rozwijający się catering dietetyczny oferujący zróżnicowane menu dla osób dbających o zdrowe odżywianie. Wyróżnia się bogatym wyborem diet i elastycznością zamówień.",
};
...
export default function SuperMenuSummary2025() {
  ...
  const { data: periodStats, isLoading: isLoadingStats } = useQuery({
    queryKey: ['supermenu-period-stats', 'v3-last-full-month'],
    queryFn: async () => {
  ...
  const { data: monthlyTrends, isLoading: isLoadingTrends } = useQuery({
    queryKey: ['supermenu-monthly-trends', 'v3-full-range'],
    queryFn: async () => {
      const allReviews = await fetchAllReviews(BRAND_ID);
  ...
  const { data: crisisMonthsData } = useQuery({
    queryKey: ['supermenu-crisis-months'],
    queryFn: async () => {
  ...
  const { data: realQuotes, isLoading: isLoadingQuotes } = useQuery({
    queryKey: ['supermenu-real-quotes'],
    queryFn: async () => {
  ...
  const { data: discountComparison, isLoading: isLoadingDiscounts } = useQuery({
    queryKey: ['supermenu-discount-comparison-q4-v2'],
    queryFn: async () => {
      ...
      // Always include SuperMenu even if no discounts
      brandAggregates['SuperMenu NEW 25'] = { sum: 0, count: 0, name: 'SuperMenu NEW 25' };
      ...
      // Calculate averages and sort (SuperMenu with 0 discounts = 0%)
  ...
  const { data: activeCodes, isLoading: isLoadingCodes } = useQuery({
    queryKey: ['supermenu-discount-codes-q4'],
    queryFn: async () => {
      const { data } = await supabase
        .from('discounts')
  ...
  const { data: priceComparison, isLoading: isLoadingPrices } = useQuery({
    queryKey: ['supermenu-price-comparison-q4'],
    queryFn: async () => {
      ...
      if (!priceData || priceData.length === 0) return { catalog: [], afterDiscount: [] };
      ...
      // Keep chart readable: show top 12, but always include SuperMenu even if outside top
      const superMenuEntry = allCatalogPrices.find(p => p.brand.includes('SuperMenu'));
      let catalogPrices = allCatalogPrices.slice(0, 12);
      if (superMenuEntry && !catalogPrices.some(p => p.brand.includes('SuperMenu'))) {
        catalogPrices = [...catalogPrices.slice(0, 11), superMenuEntry];
      }

      // For "after discount", we need to combine with discount data
      // Use the discount comparison data to estimate
  ...
  // Find SuperMenu's position in discount comparison
  const superMenuDiscount = useMemo(() => {
    if (!discountComparison) return null;
    return discountComparison.find((d: any) => d.brand.includes('SuperMenu'));
  }, [discountComparison]);

  // Find SuperMenu's position in price comparison
  const superMenuPrice = useMemo(() => {
    if (!priceComparison?.catalog) return null;
    return priceComparison.catalog.find((p: any) => p.brand.includes('SuperMenu'));
  }, [priceComparison]);
      ...
      <div className="container mx-auto px-4 py-8 space-y-10">
          ...
          <Card className="border-l-4 border-l-primary bg-primary/5">
            <CardContent className="pt-6">
              <p className="text-lg leading-relaxed">
                <strong>SuperMenu NEW 25</strong> to marka o <span className={ratingInterpretation.color}>{ratingInterpretation.text}</span> ({allTimeStats?.avgRating || '...'}/5), 
                pozycjonująca się w górnym segmencie cenowym rynku. 
                <span className="text-primary font-semibold"> Kluczowe: {parseFloat(allTimeStats?.avgRating || '0') >= 4.0 ? 'wysoka jakość produktu przy premium pricing' : 'potrzeba poprawy jakości i komunikacji wartości'}.</span>
              </p>
            </CardContent>
          </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="flex gap-3">
                  ...
                  <p className="text-sm"><strong>Średnia ocena {allTimeStats?.avgRating || '...'}/5</strong> - {parseFloat(allTimeStats?.avgRating || '0') >= 4.0 ? 'dobra reputacja' : 'wymaga poprawy'}</p>
                </div>
                <div className="flex gap-3">
                ...
                <div className="flex gap-3">
                  ...
                  <p className="text-sm"><strong>Pozycjonowanie premium</strong> - {superMenuPrice ? `${superMenuPrice.avgPrice} zł średnio` : 'wysokie ceny'}</p>
                </div>
                <div className="flex gap-3">
                  ...
                  <p className="text-sm"><strong>{allTimeStats?.negativePercent || '0'}% opinii negatywnych</strong> - główne wyzwanie</p>
                </div>
                <div className="flex gap-3">
                  ...
                  <p className="text-sm"><strong>Polityka rabatowa</strong> ({superMenuDiscount?.avgDiscount || '...'}%) vs konkurencja</p>
                </div>
              </CardContent>
            </Card>
          </div>
            ...
            <CardContent>
              <div className="grid gap-4 md:grid-cols-3">
                <div className="p-4 rounded-lg bg-blue-500/10 border border-blue-500/20">
                  <Badge className="mb-2 bg-blue-500">CENOWA</Badge>
                  <h4 className="font-semibold mb-1">Optymalizacja rabatów</h4>
                  <p className="text-sm text-muted-foreground">
                    Rozważenie większych rabatów w okresach mniejszego popytu dla zwiększenia wolumenu
                  </p>
                </div>
                <div className="p-4 rounded-lg bg-purple-500/10 border border-purple-500/20">
                  <Badge className="mb-2 bg-purple-500">KOMUNIKACYJNA</Badge>
                  <h4 className="font-semibold mb-1">Komunikacja wartości</h4>
                  <p className="text-sm text-muted-foreground">
                    Podkreślenie jakości składników uzasadniającej premium pricing
                  </p>
                </div>
                <div className="p-4 rounded-lg bg-green-500/10 border border-green-500/20">
                  <Badge className="mb-2 bg-green-500">PRODUKTOWA</Badge>
                  <h4 className="font-semibold mb-1">Rozwój menu</h4>
                  <p className="text-sm text-muted-foreground">
                    Kontynuacja rozwoju różnorodności menu jako wyróżnika
                  </p>
                </div>
              </div>
            </CardContent>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                {[
                  "Smak posiłków - wysokiej jakości dania",
                  "Jakość składników - świeże produkty",
                  "Różnorodność menu i diet",
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                {[
                  ...
                  "Mała elastyczność rabatowa",
                  "Wielkość niektórych porcji",
                  "Sporadyczne problemy z dostawą",
                  "Ograniczenia w modyfikacji menu"
                ].map((item, idx) => (
                  <div key={idx} className="flex items-start gap-2">
                      ...
                      return (
                        <div key={idx} className="flex items-center gap-4 p-4 rounded-lg bg-muted/50">
                          <div className="flex-shrink-0 w-36">
                            ...
                            <p className="text-xs text-muted-foreground">{item.stats.avgRating}★ | {item.stats.count} opinii</p>
                          </div>
                          <div className="flex-1">
                            <Progress 
                              value={parseFloat(item.stats.positivePercent)} 
                              className="h-3"
                            />
                          </div>
                          <div className="flex-shrink-0 w-32 text-right">
                            <Badge 
                              variant={emotion.type === 'positive' ? 'default' : emotion.type === 'negative' ? 'destructive' : 'secondary'}
                              className={emotion.type === 'positive' ? 'bg-green-500' : ''}
                            >
                              {emotion.dominant}
                            </Badge>
                          </div>
          ...
          <Card>
            ...
            <CardContent>
                  ...
                  <tbody>
                    <tr className="border-b">
                      ...
                      <td className="py-3 px-4 text-sm text-muted-foreground">
                        {parseFloat(allTimeStats?.avgRating || '0') >= 4.0 ? 'Stabilna reputacja w segmencie premium' : 'Reputacja wymagająca poprawy'}
                      </td>
                    </tr>
                    <tr className="border-b">
              ...
              <Card className="mt-6 border-l-4 border-l-primary bg-primary/5">
                <CardContent className="pt-4">
                  <p className="text-sm">
                    <strong>Insight strategiczny:</strong> SuperMenu {parseFloat(allTimeStats?.avgRating || '0') >= 4.0 ? 'utrzymuje stabilną reputację bez dramatycznych wahań, co świadczy o konsekwentnej jakości produktu i usług' : 'ma wyzwania z konsekwencją jakości, co widać w niższych ocenach'}.
                  </p>
                </CardContent>
              </Card>
            </CardContent>
          ...
          <Card className="border-2 border-blue-500/30">
            ...
            <CardContent className="space-y-6 pt-6">
              ...
              <Card className="border-l-4 border-l-blue-500 bg-blue-500/5">
                <CardContent className="pt-4">
                  <p className="text-sm">
                    <strong>Wniosek:</strong> SuperMenu utrzymuje {quarterStats && parseFloat(quarterStats.avgRating) >= 4.0 ? 'stabilną jakość przez cały kwartał' : 'zmienne wyniki w ostatnim kwartale'}. 
        ...
        <section className="space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              {isLoadingDiscounts ? (
                ...
                (() => {
                  // Always include SuperMenu in the chart, even if outside top 10
                  const superMenuEntry = discountComparison.find((d: any) => d.brand.includes('SuperMenu'));
                  let chartData = discountComparison.slice(0, 10);
                  if (superMenuEntry && !chartData.some((d: any) => d.brand.includes('SuperMenu'))) {
                    chartData = [...chartData.slice(0, 9), superMenuEntry].sort((a: any, b: any) => b.avgDiscount - a.avgDiscount);
                  }
                  return (
                        ...
                        >
                          ...
                          <Bar dataKey="avgDiscount" radius={[0, 4, 4, 0]}>
                            {chartData.map((entry: any, index: number) => (
                              <Cell 
                                key={`cell-${index}`} 
                                fill={entry.brand.includes('SuperMenu') ? 'hsl(var(--primary))' : 'hsl(var(--muted-foreground))'} 
                              />
                            ))}
                          </Bar>
                        </BarChart>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                Kody rabatowe SuperMenu Q4 2025
              </CardTitle>
            </CardHeader>
            <CardContent>
              {isLoadingCodes ? (
          ...
          <Card className="border-l-4 border-l-primary bg-primary/5">
            <CardContent className="pt-6">
              <p className="text-sm">
                <strong>Insight:</strong> SuperMenu stosuje {superMenuDiscount ? `średni rabat ${superMenuDiscount.avgDiscount}%` : 'konserwatywną politykę rabatową'}, 
                co w połączeniu z premium pricing daje najwyższą cenę końcową dla klienta. 
                {discountComparison && discountComparison.length > 0 && (
        ...
        <section className="space-y-6">
            ...
            <Card className="bg-gradient-to-br from-primary/10 to-primary/5 border-primary/20">
              <CardContent className="pt-6 text-center">
                <div className="text-3xl font-bold text-primary mb-1">
                  {superMenuPrice ? `${superMenuPrice.avgPrice} zł` : '...'}
                </div>
          ...
          <Card className="border-l-4 border-l-primary bg-primary/5">
            <CardContent className="pt-6">
              <p className="text-sm">
                <strong>Insight:</strong> {superMenuPrice && priceComparison?.catalog ? (
                  <>SuperMenu {superMenuPrice.avgPrice === priceComparison.catalog[0]?.avgPrice ? 'jest liderem cenowym' : 'pozycjonuje się wysoko cenowo'} 
                  ({superMenuPrice.avgPrice} zł średnio). {superMenuDiscount && (
                    <>W połączeniu z rabatem {superMenuDiscount.avgDiscount}% daje cenę końcową ok. {(superMenuPrice.avgPrice * (1 - superMenuDiscount.avgDiscount / 100)).toFixed(2)} zł.</>
                  )}</>
        ...
        <section className="space-y-6">
                ...
                <div className="p-4 rounded-lg bg-green-500/5 border border-green-500/20">
                  ...
                  <ul className="space-y-2 text-sm">
                    <li className="flex items-start gap-2">
                      ...
                      <span><strong>Średnia ocena {allTimeStats?.avgRating}/5</strong> - {parseFloat(allTimeStats?.avgRating || '0') >= 4.0 ? 'dobra' : 'do poprawy'}</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <div className="w-1.5 h-1.5 rounded-full bg-green-500 mt-1.5" />
                ...
                <div className="p-4 rounded-lg bg-orange-500/5 border border-orange-500/20">
                  ...
                  <ul className="space-y-2 text-sm">
                    <li className="flex items-start gap-2">
                      ...
                      <span><strong>{allTimeStats?.negativePercent}% opinii negatywnych</strong> - główne wyzwanie</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <div className="w-1.5 h-1.5 rounded-full bg-orange-500 mt-1.5" />
                      <span><strong>Strategia cenowa</strong> - {superMenuPrice ? `${superMenuPrice.avgPrice} zł` : 'wysokie ceny'}</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <div className="w-1.5 h-1.5 rounded-full bg-orange-500 mt-1.5" />
                      <span><strong>Polityka rabatowa</strong> - {superMenuDiscount ? `${superMenuDiscount.avgDiscount}%` : 'niskie rabaty'}</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <div className="w-1.5 h-1.5 rounded-full bg-orange-500 mt-1.5" />
                      <span><strong>Komunikacja wartości</strong> - uzasadnienie premium pricing</span>
                    </li>
                  </ul>
                </div>
              </div>
          ...
          <Card className="border-2 border-primary/30 bg-gradient-to-br from-primary/5 to-transparent">
            ...
            <CardContent className="space-y-4">
              <p className="text-sm">
                SuperMenu {parseFloat(allTimeStats?.avgRating || '0') >= 4.0 ? 'utrzymuje solidną pozycję' : 'ma potencjał poprawy'} w segmencie premium. Kluczowe rekomendacje na 2026:
              </p>
              <div className="grid gap-3 md:grid-cols-3">
                <div className="p-4 rounded-lg bg-background border">
                  <div className="text-2xl mb-2">💎</div>
                  <h5 className="font-semibold text-sm mb-1">Komunikacja wartości</h5>
                  <p className="text-xs text-muted-foreground">Wzmocnienie przekazu o jakości uzasadniającej premium pricing</p>
                </div>
                <div className="p-4 rounded-lg bg-background border">
                  <div className="text-2xl mb-2">📢</div>
                  <h5 className="font-semibold text-sm mb-1">Zbieranie opinii</h5>
                  <p className="text-xs text-muted-foreground">Aktywne zachęcanie klientów do dzielenia się opiniami</p>
                </div>
                <div className="p-4 rounded-lg bg-background border">
                  <div className="text-2xl mb-2">🎯</div>
                  <h5 className="font-semibold text-sm mb-1">Segmentacja</h5>
                  <p className="text-xs text-muted-foreground">Precyzyjne targetowanie klientów szukających jakości, nie ceny</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-l-4 border-l-primary bg-primary/5">
            <CardContent className="pt-6">
              <p className="text-sm">
                <strong>Podsumowanie:</strong> SuperMenu NEW 25 zebrało {allTimeStats?.count?.toLocaleString('pl-PL') || '...'} opinii 
                ze średnią oceną {allTimeStats?.avgRating || '...'}/5 ({allTimeStats?.positivePercent || '...'}% pozytywnych).
                <strong> Kluczowy insight: {parseFloat(allTimeStats?.avgRating || '0') >= 4.0 ? 'marka skutecznie utrzymuje pozycję premium' : 'marka ma wyzwania reputacyjne do rozwiązania'}</strong> 
                {superMenuPrice && ` przy cenach ok. ${superMenuPrice.avgPrice} zł`}
                {superMenuDiscount && ` i rabatach ${superMenuDiscount.avgDiscount}%`}.
              </p>
            </CardContent>
          </Card>
        </section>
