...
// Brand ID for Pomelo
const BRAND_ID = "98dfa2b7-1097-433f-871f-a2cbaa7ad5ec";

const companyInfo = {
  name: "Pomelo",
  brandName: "Pomelo Catering",
  businessType: "Catering dietetyczny",
  voivodeship: "Mazowieckie",
  website: "www.pomelocatering.pl",
  description: "Marka cateringu dietetycznego z długą historią na rynku (od 2017). Oferuje diety dopasowane do różnych potrzeb kalorycznych i zdrowotnych. Znana z dobrej jakości posiłków i różnorodności menu.",
};
...
export default function PomeloOpinie() {
  ...
  const { data: periodStats, isLoading: isLoadingStats } = useQuery({
    queryKey: ['pomelo-period-stats', 'v3-last-full-month'],
    queryFn: async () => {
  ...
  const { data: monthlyTrends, isLoading: isLoadingTrends } = useQuery({
    queryKey: ['pomelo-monthly-trends', 'v3-full-range'],
    queryFn: async () => {
  ...
  // Fetch crisis months data (II, VIII, XI, XII 2025)
  const { data: crisisMonthsData } = useQuery({
    queryKey: ['pomelo-crisis-months'],
    queryFn: async () => {
      const crisisMonths = ['2025-02', '2025-08', '2025-11', '2025-12'];
      ...
      for (const month of crisisMonths) {
        ...
        const endDate = month === '2025-12' ? '2025-12-31' : 
                        month === '2025-11' ? '2025-11-30' : 
                        month === '2025-08' ? '2025-08-31' : '2025-02-28';
  ...
  const { data: discountComparison, isLoading: isLoadingDiscounts } = useQuery({
    queryKey: ['pomelo-discount-comparison-q4'],
    queryFn: async () => {
      ...
      // Always include Pomelo even if no discounts
      brandAggregates['Pomelo'] = { sum: 0, count: 0, name: 'Pomelo' };
  ...
  const { data: activeCodes, isLoading: isLoadingCodes } = useQuery({
    queryKey: ['pomelo-discount-codes-q4'],
    queryFn: async () => {
      const { data } = await supabase
        .from('discounts')
  ...
  const { data: priceComparison, isLoading: isLoadingPrices } = useQuery({
    queryKey: ['pomelo-price-comparison-q4'],
    queryFn: async () => {
      // Fetch price history for Q4 2025
      ...
      if (!priceData || priceData.length === 0) return { catalog: [], afterDiscount: [], pomeloStats: null };
      ...
      // Get Pomelo stats separately
      const pomeloStats = brandAggregates['Pomelo'] ? {
        avgPrice: parseFloat((brandAggregates['Pomelo'].sum / brandAggregates['Pomelo'].count).toFixed(2)),
        minPrice: parseFloat(brandAggregates['Pomelo'].min.toFixed(2)),
        maxPrice: parseFloat(brandAggregates['Pomelo'].max.toFixed(2)),
        count: brandAggregates['Pomelo'].count
      } : null;
      ...
      // Keep chart readable: show top 12, but always include Pomelo even if outside top
      const pomeloEntry = allCatalogPrices.find(p => p.brand === 'Pomelo');
      let catalogPrices = allCatalogPrices.slice(0, 12);
      if (pomeloEntry && !catalogPrices.some(p => p.brand === 'Pomelo')) {
        catalogPrices = [...catalogPrices.slice(0, 11), pomeloEntry];
      }
      ...
      return { catalog: catalogPrices, afterDiscount: afterDiscountPrices, pomeloStats };
    },
    enabled: !!discountComparison // Wait for discount data
  });

  // Find Pomelo's discount stats
  const pomeloDiscount = useMemo(() => {
    if (!discountComparison) return null;
    return discountComparison.find((d: any) => d.brand === 'Pomelo');
  }, [discountComparison]);

  // Pomelo discount stats from Q4
  const pomeloDiscountStats = useMemo(() => {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8 space-y-10">
          ...
          <Card className="border-l-4 border-l-primary bg-primary/5">
            <CardContent className="pt-6">
              <p className="text-lg leading-relaxed">
                <strong>Pomelo Catering</strong> to marka o dobrej, stabilnej reputacji ({allTimeStats?.avgRating || '4.08'}/5), 
                z długą historią na rynku (od 2017). W 2025 roku marka doświadczyła kilku okresów kryzysowych 
                (luty, sierpień, XI-XII) związanych z problemami logistycznymi.
                <span className="text-primary font-semibold"> Kluczowe: styczeń 2026 pokazuje pełne odbicie - 5.0/5!</span>
              </p>
            </CardContent>
          </Card>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Na podstawie {allTimeStats?.count || '989'} opinii</CardDescription>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="flex gap-3">
                  ...
                  <p className="text-sm"><strong>Długa historia</strong> - marka działa od 2017 roku z konsekwentną jakością</p>
                </div>
                <div className="flex gap-3">
                  ...
                  <p className="text-sm"><strong>Smak i świeżość</strong> - najczęściej chwalony aspekt przez klientów</p>
                </div>
                <div className="flex gap-3">
                  ...
                  <p className="text-sm"><strong>Odporność na kryzysy</strong> - marka szybko wraca po okresach problemów</p>
                </div>
                <div className="flex gap-3">
                  ...
                  <p className="text-sm"><strong>Wahania sezonowe</strong> - wyraźne spadki w lutym, sierpniu, XI-XII</p>
                </div>
                <div className="flex gap-3">
                  ...
                  <p className="text-sm"><strong>Pełne odbicie I 2026</strong> - 18 opinii po 5.0/5 pokazuje powrót formy</p>
                </div>
              </CardContent>
            </Card>
          </div>
              ...
              <div className="grid gap-4 md:grid-cols-3">
                <div className="p-4 rounded-lg bg-blue-500/10 border border-blue-500/20">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Przygotowanie na okresy zwiększonego popytu (luty, sierpień, święta)
                  </p>
                </div>
                <div className="p-4 rounded-lg bg-purple-500/10 border border-purple-500/20">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Skrócenie czasu reakcji na reklamacje i zapytania
                  </p>
                </div>
                <div className="p-4 rounded-lg bg-green-500/10 border border-green-500/20">
                  <Badge className="mb-2 bg-green-500">MARKETING</Badge>
                  <h4 className="font-semibold mb-1">Wykorzystanie historii</h4>
                  <p className="text-sm text-muted-foreground">
                    Komunikacja wieloletniego doświadczenia i stabilności marki
                  </p>
                </div>
              </div>
            </CardContent>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                {[
                  "Smak posiłków - \"pyszne\", \"świeże\", \"zdrowe\"",
                  "Różnorodność menu i możliwość wyboru",
                  "Punktualność dostaw w stabilnych okresach",
                  "Efekty zdrowotne - utrata wagi, lepsze samopoczucie",
                  "Estetyka i jakość opakowań",
                  "Długoletnie doświadczenie marki"
                ].map((item, idx) => (
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                {[
                  "Problemy z dostawami w okresach szczytowych",
                  "Brak lub opóźniona reakcja na reklamacje",
                  "Niespójna jakość w niektórych miesiącach",
                  "Małe porcje w stosunku do ceny",
                  "Problemy z kontaktem telefonicznym",
                  "Długi czas oczekiwania na zwroty"
                ].map((item, idx) => (
                  <div key={idx} className="flex items-start gap-2">
                      ...
                      return (
                        <div key={idx} className="flex items-center gap-4 p-4 rounded-lg bg-muted/50">
                          ...
                          <div className="text-right">
                            <p className="text-lg font-bold">{item.stats.avgRating} ★</p>
                            <p className="text-xs text-muted-foreground">
                              {item.stats.positivePercent}% poz.
                            </p>
                          </div>
                        </div>
                      );
        ...
        {/* Sekcja 3: Aspekty opinii */}
        <ReviewAspectsSection />

        <Separator />

        {/* Sekcja 3.5: Elementy marki - teraz używa ReviewAspectsSection */}
        <ReviewAspectsSection />
        ...
        <section className="space-y-6">
          ...
          <Card>
            ...
            <CardContent>
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    <tr className="border-b">
                      ...
                      <td className="py-3 px-4 text-sm text-muted-foreground">Stabilna, dobra reputacja od 2017</td>
                    </tr>
                    <tr className="border-b">
                      ...
                      <td className="py-3 px-4 text-sm text-muted-foreground">Wahania z odbiciem w I 2026</td>
                    </tr>
                    <tr className="border-b">
                      ...
                      <td className="py-3 px-4 text-sm text-muted-foreground">Problemy XI-XII, odbicie w I</td>
                    </tr>
                    <tr>
                      ...
                      <td className="py-3 px-4 text-sm text-muted-foreground">Pełne odbicie - najlepszy wynik!</td>
                    </tr>
                  </tbody>
                </table>
              </div>
              ...
              <Card className="mt-6 border-l-4 border-l-primary bg-primary/5">
                <CardContent className="pt-4">
                  <p className="text-sm">
                    <strong>Insight strategiczny:</strong> Pomelo wykazuje charakterystyczny wzorzec "V" - 
                    spadki w okresach kryzysowych, ale szybkie odbicia. Styczeń 2026 z oceną 5.0/5 (18 opinii) 
                    potwierdza zdolność marki do odbudowy.
                  </p>
                </CardContent>
              </Card>
            </CardContent>
          ...
          {/* Deep Dive: Crisis months */}
          <Card className="border-2 border-orange-500/30">
            <CardHeader className="bg-orange-500/5">
              <CardTitle className="flex items-center gap-2">
                <AlertTriangle className="h-5 w-5 text-orange-500" />
                Deep Dive: Miesiące kryzysowe 2025
              </CardTitle>
              <CardDescription>Szczegółowa analiza okresów z niskimi ocenami</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6 pt-6">
              ...
              <div>
                ...
                <div className="grid gap-3 md:grid-cols-4">
                  <div className="p-4 rounded-lg bg-red-500/10 border border-red-500/30">
                    <div className="text-sm text-muted-foreground">Luty 2025</div>
                    <div className="text-2xl font-bold text-red-600">
                      {crisisMonthsData?.['2025-02']?.avgRating?.toFixed(2) || '2.00'} ★
                    </div>
                    <div className="text-xs text-muted-foreground mt-1">
                      {crisisMonthsData?.['2025-02']?.count || 4} opinii
                    </div>
                  </div>
                  <div className="p-4 rounded-lg bg-orange-500/10 border border-orange-500/30">
                    <div className="text-sm text-muted-foreground">Sierpień 2025</div>
                    <div className="text-2xl font-bold text-orange-600">
                      {crisisMonthsData?.['2025-08']?.avgRating?.toFixed(2) || '3.00'} ★
                    </div>
                    <div className="text-xs text-muted-foreground mt-1">
                      {crisisMonthsData?.['2025-08']?.count || 8} opinii
                    </div>
                  </div>
                  <div className="p-4 rounded-lg bg-red-500/10 border border-red-500/30">
                    <div className="text-sm text-muted-foreground">Listopad 2025</div>
                    <div className="text-2xl font-bold text-red-600">
                      {crisisMonthsData?.['2025-11']?.avgRating?.toFixed(2) || '2.33'} ★
                    </div>
                    <div className="text-xs text-muted-foreground mt-1">
                      {crisisMonthsData?.['2025-11']?.count || 3} opinii
                    </div>
                  </div>
                  <div className="p-4 rounded-lg bg-red-500/10 border border-red-500/30">
                    <div className="text-sm text-muted-foreground">Grudzień 2025</div>
                    <div className="text-2xl font-bold text-red-600">
                      {crisisMonthsData?.['2025-12']?.avgRating?.toFixed(2) || '1.67'} ★
                    </div>
                    <div className="text-xs text-muted-foreground mt-1">
                      {crisisMonthsData?.['2025-12']?.count || 6} opinii
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div>
                <h4 className="font-semibold mb-4">Główne problemy w okresach kryzysowych</h4>
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="w-16 text-sm text-right font-medium text-muted-foreground">5%</div>
                  </div>
                </div>
              </div>
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <div className="p-3 rounded-lg bg-muted/50">
                    ...
                    <ul className="text-sm space-y-1 text-muted-foreground">
                      ...
                      <li>• Opóźnienia w dostawach</li>
                      <li>• Problemy z komunikacją o zmianach</li>
                    </ul>
                  </div>
                  <div className="p-3 rounded-lg bg-muted/50">
                    <div className="flex items-center gap-2 mb-2">
                    ...
                    <ul className="text-sm space-y-1 text-muted-foreground">
                      ...
                      <li>• Długi czas oczekiwania na zwroty</li>
                      <li>• Problemy z kontaktem telefonicznym</li>
                    </ul>
                  </div>
                </div>
              </div>
              ...
              <Card className="border-l-4 border-l-orange-500 bg-orange-500/5">
                <CardContent className="pt-4">
                  <p className="text-sm">
                    <strong>Wniosek:</strong> Kryzysy 2025 miały charakter wyłącznie operacyjny - produkt pozostaje wysoko oceniany. 
                    Styczeń 2026 z oceną 5.0/5 (18 opinii) potwierdza, że marka potrafi szybko wrócić do formy po naprawie procesów.
                    <strong> Kluczowe: wzorzec "V" - spadek i szybkie odbicie.</strong>
                  </p>
                </CardContent>
              </Card>
            </CardContent>
        ...
        <section className="space-y-6">
            ...
            <>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Ostatnie opinie z oceną 4-5★</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  {realQuotes?.positive && realQuotes.positive.length > 0 ? (
                    realQuotes.positive.slice(0, 4).map((quote, idx) => (
                      <div key={idx} className="p-4 rounded-lg bg-green-500/5 border-l-4 border-l-green-500">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {quote.review_date ? format(new Date(quote.review_date), 'LLLL yyyy', { locale: pl }) : 'Brak daty'}
                        </p>
                      </div>
                    ))
                  ) : (
                    <p className="text-muted-foreground text-sm">Brak pozytywnych cytatów do wyświetlenia</p>
                  )}
                </CardContent>
              </Card>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Ostatnie opinie z oceną 1-2★</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  {realQuotes?.negative && realQuotes.negative.length > 0 ? (
                    ...
                    <p className="text-muted-foreground text-sm">Brak negatywnych cytatów do wyświetlenia</p>
                  )}
                </CardContent>
              </Card>
            </>
          ...
          <Card className="border-l-4 border-l-primary bg-primary/5">
            <CardContent className="pt-6">
              ...
              <p className="text-sm text-muted-foreground">
                Cytaty pokazują jak zmienia się narracja klientów, nie tylko liczby. W okresach stabilnych dominuje 
                satysfakcja z jakości i smaku. W okresach kryzysowych - frustracja procesowa, ale nadal uznanie 
                dla jakości produktu. To kluczowy insight: <strong>problem jest operacyjny, nie produktowy</strong>.
              </p>
            </CardContent>
          </Card>
        </section>
        ...
        <section className="space-y-6">
            ...
            <Card className="bg-gradient-to-br from-primary/10 to-primary/5 border-primary/20">
              <CardContent className="pt-6 text-center">
                <div className="text-3xl font-bold text-primary mb-1">
                  {isLoadingCodes ? <Skeleton className="h-8 w-8 mx-auto" /> : pomeloDiscountStats.count}
                </div>
            ...
            <Card className="bg-gradient-to-br from-green-500/10 to-green-500/5 border-green-500/20">
              <CardContent className="pt-6 text-center">
                <div className="text-3xl font-bold text-green-600 mb-1">
                  {isLoadingCodes ? <Skeleton className="h-8 w-12 mx-auto" /> : `${pomeloDiscountStats.avg}%`}
                </div>
            ...
            <Card className="bg-gradient-to-br from-orange-500/10 to-orange-500/5 border-orange-500/20">
              <CardContent className="pt-6 text-center">
                <div className="text-3xl font-bold text-orange-600 mb-1">
                  {isLoadingCodes ? <Skeleton className="h-8 w-12 mx-auto" /> : `${pomeloDiscountStats.max}%`}
                </div>
            ...
            <Card className="bg-gradient-to-br from-blue-500/10 to-blue-500/5 border-blue-500/20">
              <CardContent className="pt-6 text-center">
                <div className="text-3xl font-bold text-blue-600 mb-1">
                  {isLoadingCodes ? <Skeleton className="h-8 w-12 mx-auto" /> : `${pomeloDiscountStats.min}%`}
                </div>
          ...
          <Card>
            ...
            <CardContent>
              {isLoadingDiscounts ? (
                ...
                (() => {
                  // Always include Pomelo in the chart, even if outside top 10
                  const pomeloEntry = discountComparison.find((d: any) => d.brand === 'Pomelo');
                  let chartData = discountComparison.slice(0, 10);
                  if (pomeloEntry && !chartData.some((d: any) => d.brand === 'Pomelo')) {
                    chartData = [...chartData.slice(0, 9), pomeloEntry].sort((a: any, b: any) => b.avgDiscount - a.avgDiscount);
                  }
                  return (
                        ...
                        >
                          ...
                          <Bar dataKey="avgDiscount" radius={[0, 4, 4, 0]}>
                            {chartData.map((entry: any, index: number) => (
                              <Cell 
                                key={`cell-${index}`} 
                                fill={entry.brand === 'Pomelo' ? 'hsl(var(--primary))' : 'hsl(var(--muted-foreground))'} 
                              />
                            ))}
                          </Bar>
                        </BarChart>
                  ...
                  {activeCodes.map((code, idx) => (
                    <div key={idx} className="p-3 rounded-lg bg-muted/50 border">
                      <div className="flex justify-between items-start mb-2">
                        <code className="font-mono font-bold">{code.code || 'Bez kodu'}</code>
          ...
          <Card className="border-l-4 border-l-primary bg-primary/5">
            <CardContent className="pt-6">
              <p className="text-sm">
                <strong>Insight:</strong> Pomelo stosuje <strong>zbalansowaną politykę rabatową</strong> - średni rabat {pomeloDiscountStats.avg}% 
                {pomeloDiscount && discountComparison && (
                  <> (pozycja {discountComparison.findIndex((d: any) => d.brand === 'Pomelo') + 1} z {discountComparison.length})</>
                )}. Strategia łączy regularne promocje sezonowe z ciągłymi kodami dla nowych i powracających klientów.
              </p>
            </CardContent>
          </Card>
        </section>
        ...
        <section className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-4">
            <Card className="bg-gradient-to-br from-primary/10 to-primary/5 border-primary/20">
              <CardContent className="pt-6 text-center">
                <div className="text-3xl font-bold text-primary mb-1">
                  {isLoadingPrices ? <Skeleton className="h-8 w-20 mx-auto" /> : `${priceComparison?.pomeloStats?.avgPrice || '...'} zł`}
                </div>
            ...
            <Card className="bg-gradient-to-br from-green-500/10 to-green-500/5 border-green-500/20">
              <CardContent className="pt-6 text-center">
                <div className="text-3xl font-bold text-green-600 mb-1">
                  {isLoadingPrices || isLoadingDiscounts ? (
                    <Skeleton className="h-8 w-20 mx-auto" />
                  ) : (
                    `~${(priceComparison?.pomeloStats?.avgPrice ? priceComparison.pomeloStats.avgPrice * (1 - pomeloDiscountStats.avg / 100) : 0).toFixed(2)} zł`
                  )}
                </div>
                <div className="text-sm text-muted-foreground">Śr. cena po rabacie {pomeloDiscountStats.avg}%</div>
              </CardContent>
            </Card>
            <Card className="bg-gradient-to-br from-blue-500/10 to-blue-500/5 border-blue-500/20">
              <CardContent className="pt-6 text-center">
                <div className="text-3xl font-bold text-blue-600 mb-1">
                  {isLoadingPrices ? <Skeleton className="h-8 w-20 mx-auto" /> : `${priceComparison?.pomeloStats?.minPrice || '...'} zł`}
                </div>
            ...
            <Card className="bg-gradient-to-br from-orange-500/10 to-orange-500/5 border-orange-500/20">
              <CardContent className="pt-6 text-center">
                <div className="text-3xl font-bold text-orange-600 mb-1">
                  {isLoadingPrices ? <Skeleton className="h-8 w-20 mx-auto" /> : `${priceComparison?.pomeloStats?.maxPrice || '...'} zł`}
                </div>
            ...
            <CardContent>
              {isLoadingPrices ? (
                ...
                <div className="h-[350px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart 
                      data={priceComparison.catalog.slice(0, 12)}
                      layout="vertical"
                      margin={{ top: 5, right: 30, left: 110, bottom: 5 }}
                    >
                      <CartesianGrid strokeDasharray="3 3" />
                      ...
                      <Bar dataKey="avgPrice" radius={[0, 4, 4, 0]}>
                        {priceComparison.catalog.slice(0, 12).map((entry: any, index: number) => (
                          <Cell 
                            key={`cell-${index}`} 
                            fill={entry.brand === 'Pomelo' ? 'hsl(var(--primary))' : 'hsl(var(--muted-foreground))'} 
                          />
            ...
            <CardContent>
              {isLoadingPrices ? (
                ...
                <div className="h-[350px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart 
                      data={priceComparison.afterDiscount.slice(0, 12)}
                      layout="vertical"
                      margin={{ top: 5, right: 30, left: 110, bottom: 5 }}
                    >
                      <CartesianGrid strokeDasharray="3 3" />
                      ...
                      <Bar dataKey="avgPrice" radius={[0, 4, 4, 0]}>
                        {priceComparison.afterDiscount.slice(0, 12).map((entry: any, index: number) => (
                          <Cell 
                            key={`cell-${index}`} 
                            fill={entry.brand === 'Pomelo' ? 'hsl(var(--primary))' : 'hsl(var(--muted-foreground))'} 
                          />
          ...
          <Card className="border-l-4 border-l-primary bg-primary/5">
            <CardContent className="pt-6">
              <p className="text-sm">
                <strong>Insight:</strong> Pomelo pozycjonuje się w <strong>segmencie cenowym</strong> ({priceComparison?.pomeloStats?.avgPrice || '...'} zł),
                co w połączeniu z {pomeloDiscountStats.avg}% rabatem daje atrakcyjną cenę końcową ~{(priceComparison?.pomeloStats?.avgPrice ? priceComparison.pomeloStats.avgPrice * (1 - pomeloDiscountStats.avg / 100) : 0).toFixed(2)} zł. 
                To plasuje markę jako jedną z bardziej przystępnych opcji dla klientów szukających balansu między jakością a ceną.
              </p>
            </CardContent>
          </Card>
        </section>
        ...
        <section className="space-y-6">
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="flex gap-4 items-start">
                  <div className="w-24 flex-shrink-0 font-semibold text-sm text-green-600">I 2025</div>
                  <div className="flex-1 p-3 rounded-lg bg-green-500/5 border-l-4 border-l-green-500">
                    <p className="text-sm font-medium">Doskonały start roku</p>
                    <p className="text-xs text-muted-foreground mt-1">Średnia ocena 4.81 | 16 opinii | Bardzo pozytywny odbiór</p>
                  </div>
                </div>
                <div className="flex gap-4 items-start">
                  <div className="w-24 flex-shrink-0 font-semibold text-sm text-red-600">II 2025</div>
                  <div className="flex-1 p-3 rounded-lg bg-red-500/5 border-l-4 border-l-red-500">
                    <p className="text-sm font-medium">Pierwszy kryzys</p>
                    <p className="text-xs text-muted-foreground mt-1">Średnia ocena 2.00 | 4 opinii | Problemy z dostawami</p>
                  </div>
                </div>
                <div className="flex gap-4 items-start">
                  <div className="w-24 flex-shrink-0 font-semibold text-sm text-green-600">III-VII 2025</div>
                  <div className="flex-1 p-3 rounded-lg bg-green-500/5 border-l-4 border-l-green-500">
                    <p className="text-sm font-medium">Stabilny okres wzrostu</p>
                    <p className="text-xs text-muted-foreground mt-1">Średnia ocena 4.0-4.9 | Odbudowa reputacji | Dobra jakość</p>
                  </div>
                </div>
                <div className="flex gap-4 items-start">
                  <div className="w-24 flex-shrink-0 font-semibold text-sm text-orange-600">VIII 2025</div>
                  <div className="flex-1 p-3 rounded-lg bg-orange-500/5 border-l-4 border-l-orange-500">
                    <p className="text-sm font-medium">Letni spadek</p>
                    <p className="text-xs text-muted-foreground mt-1">Średnia ocena 3.00 | 8 opinii | Problemy z jakością i obsługą</p>
                  </div>
                </div>
                <div className="flex gap-4 items-start">
                  <div className="w-24 flex-shrink-0 font-semibold text-sm text-green-600">IX-X 2025</div>
                  <div className="flex-1 p-3 rounded-lg bg-green-500/5 border-l-4 border-l-green-500">
                    <p className="text-sm font-medium">Szybka odbudowa</p>
                    <p className="text-xs text-muted-foreground mt-1">Średnia ocena 4.33 | Powrót do formy | Dobre opinie</p>
                  </div>
                </div>
                <div className="flex gap-4 items-start">
                  <div className="w-24 flex-shrink-0 font-semibold text-sm text-red-600">XI-XII 2025</div>
                  <div className="flex-1 p-3 rounded-lg bg-red-500/5 border-l-4 border-l-red-500">
                    <p className="text-sm font-medium">Kryzys świąteczny</p>
                    <p className="text-xs text-muted-foreground mt-1">Średnia ocena 1.67-2.33 | 9 opinii | Problemy logistyczne</p>
                  </div>
                </div>
              </div>
            </CardContent>
                ...
                <div className="p-4 rounded-lg bg-green-500/5 border border-green-500/20">
                  ...
                  <ul className="space-y-2 text-sm">
                    <li className="flex items-start gap-2">
                      ...
                      <span><strong>Smak i jakość posiłków</strong> - konsekwentnie najlepiej oceniany aspekt</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <div className="w-1.5 h-1.5 rounded-full bg-green-500 mt-1.5" />
                      <span><strong>Długa historia</strong> - działanie od 2017 buduje zaufanie</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <div className="w-1.5 h-1.5 rounded-full bg-green-500 mt-1.5" />
                      <span><strong>Zdolność do odbudowy</strong> - szybkie odbicia po kryzysach</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <div className="w-1.5 h-1.5 rounded-full bg-green-500 mt-1.5" />
                      <span><strong>Różnorodność menu</strong> - szeroki wybór diet</span>
                    </li>
                  </ul>
                </div>
                <div className="p-4 rounded-lg bg-red-500/5 border border-red-500/20">
                  ...
                  <ul className="space-y-2 text-sm">
                    <li className="flex items-start gap-2">
                      ...
                      <span><strong>Skalowalność logistyki</strong> - problemy w okresach szczytowych</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <div className="w-1.5 h-1.5 rounded-full bg-red-500 mt-1.5" />
                    ...
                    <li className="flex items-start gap-2">
                      <div className="w-1.5 h-1.5 rounded-full bg-red-500 mt-1.5" />
                      <span><strong>Proces zwrotów</strong> - klienci czekają zbyt długo</span>
                    </li>
                  </ul>
                </div>
              </div>
          ...
          <Card className="border-2 border-primary/30 bg-gradient-to-br from-primary/5 to-transparent">
            ...
            <CardContent className="space-y-4">
              <p className="text-sm">
                Rok 2025 zakończył się trudnym Q4, ale <strong>styczeń 2026 (5.0/5, 18 opinii)</strong> pokazuje pełne odbicie. 
              ...
              <div className="grid gap-3 md:grid-cols-3">
                <div className="p-4 rounded-lg bg-background border">
                  ...
                  <p className="text-xs text-muted-foreground">Przygotowanie na luty, sierpień i sezon świąteczny 2026</p>
                </div>
                <div className="p-4 rounded-lg bg-background border">
                  <div className="text-2xl mb-2">📞</div>
                  <h5 className="font-semibold text-sm mb-1">Obsługa klienta</h5>
                  <p className="text-xs text-muted-foreground">Skrócenie czasu reakcji i szybsze zwroty</p>
                </div>
                <div className="p-4 rounded-lg bg-background border">
                  <div className="text-2xl mb-2">📊</div>
                  <h5 className="font-semibold text-sm mb-1">Monitoring</h5>
                  <p className="text-xs text-muted-foreground">Wczesne wykrywanie spadków przed eskalacją</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-l-4 border-l-primary bg-primary/5">
            <CardContent className="pt-6">
              <p className="text-sm">
                <strong>Podsumowanie:</strong> Pomelo Catering zebrało {allTimeStats?.count?.toLocaleString('pl-PL') || '...'} opinii 
                ze średnią oceną {allTimeStats?.avgRating || '...'}/5 (od 2017 roku). 
                <strong> Kluczowy insight: marka ma charakterystyczny wzorzec "V"</strong> - spadki w okresach kryzysowych (II, VIII, XI-XII), 
                ale szybkie odbicia ({allTimeStats?.positivePercent || '...'}% pozytywnych opinii ogółem). 
                Styczeń 2026 z oceną 5.0/5 (18 opinii) potwierdza zdolność do pełnej odbudowy.
              </p>
            </CardContent>
          </Card>
        </section>
